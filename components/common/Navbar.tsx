...
const PersonSelector: React.FC = () => {
  ...
  useEffect(() => {
    if (people.length === 1 && selectedPersonId !== people[0].id) {
      selectPerson(people[0].id);
    }
    if (people.length > 1 && selectedPersonId === people[0].id) {
      selectPerson('all');
    }
  }, [people, selectedPersonId, selectPerson]);
  ...
  return (
    <div className="mb-8">
      ...
      {people.length > 1 ? (
      ...
      ) : people.length === 1 && selectedPerson && (
        <NavLink to="/settings" className="block cursor-pointer group" aria-label="Vai alle impostazioni">
          <div className="flex items-center mt-4 p-2 bg-gray-100 dark:bg-gray-700/50 rounded-lg group-hover:bg-gray-200 dark:group-hover:bg-gray-700 transition-colors">
            <img src={selectedPerson.avatar || defaultAvatar} alt={selectedPerson.name} className="w-10 h-10 rounded-full" />
            <div className="ml-3">
