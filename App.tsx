...
const AppContent: React.FC = () => {
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-100 dark:bg-gray-900">
        <div className="text-center p-8 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">Assicurati che il JSON Server sia in esecuzione</p>
  ...
  if (needsOnboarding) {
    return <OnboardingFlow isOpen={true} onComplete={completeOnboarding} />;
  }
    ...
    <BrowserRouter>
      <div style={{ "--theme-color": themeColor } as React.CSSProperties}>
        <ResponsiveLayout>
          ...
          {/* Pulsante aggiungi transazione - posizionamento responsive */}
          <button
            onClick={() => setIsModalOpen(true)}
            style={{ backgroundColor: "var(--theme-color)" }}
            className="fixed bottom-24 right-4 lg:bottom-8 lg:right-8 text-white rounded-full p-3 lg:p-4 shadow-lg transition-all transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--theme-color)] dark:focus:ring-offset-gray-900 z-40"
            aria-label="Add new transaction"
          >
            <PlusIcon className="w-6 h-6 lg:w-8 lg:h-8" />
          </button>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            <h2 className="mt-6 text-3xl font-extrabold text-gray-900 dark:text-white">Wealth Pillar</h2>
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Accedi al tuo account</p>
          </div>
          <SignIn />
        </div>
      </div>
