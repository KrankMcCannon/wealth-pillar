...
export default function OnboardingModal({
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <Input
          ...
          disabled={loading}
          className="h-11 bg-white border border-primary/20"
        />
      </div>
      <div className="space-y-2">
  ...
  const addAccount = () => {
    setAccounts((prev) => [...prev, { name: '', type: 'payroll' }]);
  };
      ...
      {accounts.map((account, index) => (
        <div key={`account-${index}`} className={onboardingStyles.card}>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={account.type}
              onValueChange={(value) => updateAccountField(index, 'type', value)}
              disabled={loading}
            >
              <SelectTrigger>
    ...
    <div className="space-y-4">
      ...
      {!categoriesLoading && categories.length === 0 && (
        <div className={onboardingStyles.warningMessage}>
          Nessuna categoria disponibile al momento. Prova ad aggiornare la pagina dopo aver completato l&apos;onboarding.
        </div>
      )}
      {budgets.map((budget, index) => (
        ...
        <div className="space-y-2">
            ...
            >
              <SelectTrigger>
                <SelectValue placeholder={categoryOptions.length ? 'Scegli una categoria' : 'Nessuna categoria disponibile'} />
              </SelectTrigger>
              <SelectContent>
      ...
      >
        <header className={onboardingStyles.header.container}>
          <div className={onboardingStyles.header.content}>
            ...
            <div>
              <p className={onboardingStyles.header.meta}>Step {currentStep + 1} di {steps.length}</p>
        ...
        <div className={onboardingStyles.footer}>
          {currentStep > 0 ? (
            <Button
              ...
              onClick={handleBack}
              disabled={loading}
              className={onboardingStyles.secondaryButton}
            >
