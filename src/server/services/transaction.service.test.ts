...
describe('TransactionService', () => {
  ...
  describe('updateTransaction', () => {
    it('should throw error when transaction ID is empty', async () => {
      await expect(
        TransactionService.updateTransaction('', { description: 'Updated' })
  ...
  describe('deleteByAccount', () => {
    ...
    it('should throw error when delete fails', async () => {
      ...
      await expect(TransactionService.deleteByAccount('account-1')).rejects.toThrow(
        'Delete failed'
      );
    });
  });
  ...
  describe('RPC Functions', () => {
    ...
    it('getGroupUserCategorySpending should call RPC and return data', async () => {
      const mockData = [
        { user_id: 'user-1', category: 'food', spent: 300, income: 0, transaction_count: 3 },
      ];
  ...
  describe('Database error handling', () => {
    it('getByGroupDb should throw on database error', async () => {
      ...
      await expect(TransactionService.getTransactionsByGroup('group-1')).rejects.toThrow(
        'DB error'
      );
    });
  ...
  describe('getByUserDb error handling', () => {
    it('should throw error when database query fails', async () => {
      const chainable = createUserChainableMock({
        data: null,
        count: null,
        error: { message: 'User query failed' },
      });
      ...
      await expect(TransactionService.getTransactionsByUser('user-1')).rejects.toThrow(
        'User query failed'
      );
    });
  });
      ...
      vi.mocked(supabase.from).mockReturnValue({
        select: vi.fn().mockReturnValue({
          or: vi.fn().mockReturnValue({
            order: vi
              .fn()
              .mockResolvedValue({ data: null, error: { message: 'Account query failed' } }),
          }),
        }),
      } as never);

      await expect(TransactionService.getTransactionsByAccount('account-1')).rejects.toThrow(
  ...
  describe('getTransactionCountByUser error handling', () => {
    it('should throw error when count query fails', async () => {
      ...
      await expect(TransactionService.getTransactionCountByUser('user-1')).rejects.toThrow(
        'Count failed'
      );
    });
        ...
        delete: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            select: vi.fn().mockReturnValue({
              single: vi
                .fn()
                .mockResolvedValue({ data: null, error: { message: 'Delete operation failed' } }),
            }),
          }),
        }),
      } as never);

      await expect(TransactionService.deleteTransaction('tx-1')).rejects.toThrow(
      ...
      vi.mocked(supabase.from).mockReturnValue({
        ...
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            single: vi
              .fn()
              .mockResolvedValue({ data: null, error: { message: 'Account not found' } }),
          }),
        }),
      } as never);
