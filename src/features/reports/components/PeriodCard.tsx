  ...
  return (
    <div className={reportsStyles.periods.card}>
      <div className={reportsStyles.periods.cardHeader}>
        ...
        <div className={reportsStyles.periods.cardHeaderMetrics}>
        </div>
      </div>
      ...
      <div className={reportsStyles.periods.cardContent}>
        ...
        <div className={reportsStyles.periods.globalMetrics}>
          <div className={reportsStyles.periods.globalMetricIncome}>
            ...
            <span className={reportsStyles.periods.globalValueIncome}>+{formatCurrency(period.totalEarned)}</span>
          </div>
          <div className={reportsStyles.periods.globalMetricExpense}>
            <span className={reportsStyles.periods.globalLabel}>Expenses</span>
            <span className={reportsStyles.periods.globalValueExpense}>-{formatCurrency(period.totalSpent)}</span>
          </div>
          <div className={reportsStyles.periods.globalMetricNet}>
            <span className={reportsStyles.periods.globalLabel}>Net Savings</span>
            <span className={reportsStyles.periods.globalValueNet}>{formatCurrency(globalNet)}</span>
          </div>
        </div>
        ...
        <div className={reportsStyles.periods.breakdownContainer}>
          ...
          <div className={reportsStyles.periods.breakdownList}>
            {sortAccountMetrics(Object.entries(period.metricsByAccountType))
              .map(([type, metrics]) => {
                  ...
                  <div key={type} className={reportsStyles.periods.breakdownItem}>
                    ...
                    <div className={reportsStyles.periods.breakdownFlows}>
                      <div className="flex items-center gap-1">
                        <ArrowUpCircle className={`w-3 h-3 ${metrics.earned > 0 ? "text-emerald-500/70" : "text-emerald-500/30"}`} />
                        <span className={`${reportsStyles.periods.breakdownIn} ${metrics.earned === 0 ? "opacity-50" : ""}`}>{formatCurrency(metrics.earned)}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <ArrowDownCircle className={`w-3 h-3 ${metrics.spent > 0 ? "text-red-500/70" : "text-red-500/30"}`} />
                        <span className={`${reportsStyles.periods.breakdownOut} ${metrics.spent === 0 ? "opacity-50" : ""}`}>{formatCurrency(metrics.spent)}</span>
                      </div>
                    </div>
