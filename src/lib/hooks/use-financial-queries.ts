...
export const useBudgetPerformance = (budgetId: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      budget.categories.forEach((category: string | number) => {
...
export const useTransactionTrends = (userId?: string, days: number = 30) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const totalSpent = Object.values(dailySpending as Record<string, number>).reduce((sum, amount) => sum + amount, 0);
      const averageDailySpending = totalSpent / days;

      // Simple trend calculation (last 7 days vs previous 7 days)
      const recent7Days = Object.entries(dailySpending as Record<string, number>)
        .slice(-7)
        .reduce((sum, [, amount]) => sum + amount, 0);

      const previous7Days = Object.entries(dailySpending as Record<string, number>)
        .slice(-14, -7)
        .reduce((sum, [, amount]) => sum + amount, 0);
