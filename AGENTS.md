...
**Operations**:
- `getAllCategories()` - Get all categories sorted alphabetically
- `getCategoryById(categoryId)` - Get category by ID
- `getCategoryByKey(key)` - Get category by unique key
- `groupCategories(categories)` - Group categories alphabetically
- `filterCategories(categories, searchQuery)` - Filter categories by search query
- `getCategoryColor(categories, categoryId)` - Helper to get category color
- `getCategoryIcon(categories, categoryId)` - Helper to get category icon

**Use Cases**:
- Category selection in transaction forms
- Category filters in reports
- Category-based spending analysis
- Budget category assignment
- UI color and icon display

**Cache Strategy**:
- TTL: 900 seconds (15 minutes)
- Tags: `categories`, `category:{categoryId}`, `category:key:{key}`
- Longer TTL since categories rarely change

**Business Logic Methods**:
Pure functions for category organization:
- `groupCategories()` - Client-side alphabetical grouping
- `filterCategories()` - Client-side search filtering
- `getCategoryColor()` - Quick color lookup
- `getCategoryIcon()` - Quick icon lookup
...
- ✅ **Service Layer** (`src/lib/services/`)
  ...
  - `CategoryService` - Category operations and helper utilities
...
- ✅ **CategoryService** - Category operations, filtering, helpers ✅
