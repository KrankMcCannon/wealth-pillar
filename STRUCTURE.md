...
| Onboarding | `/onboarding` | Name the group, add at least one account, optionally a first budget. | wizard | scannable | Continue / Finish | Stacked labels; account type radio cards; skip on budgets |
...
| Add investment | investments overlay | Record a holding. | drawer | scannable | Save | Combobox (share), amount, date, currency radios |
...
| Account form | accounts overlay | Create or edit one account. | drawer | scannable | Save account | Text, radio cards (type), radios (liquidity) |
| Budget form | budgets overlay | Create or edit one category budget. | drawer | scannable | Save budget | Category, amount, cadence radios |
...
| Filters | transactions overlay | Narrow the ledger. | drawer | scannable | Live apply (close sheet) | Exclusive period chips; custom range = two DateFields (calendar overlays the sheet); labelled account/category search. Clear is a tertiary text control on the ledger toolbar. |
| Date picker | forms | Set a single date. | drawer or inline | compact | Confirm date | Calendar. Drawer from a closed trigger; inline when the parent is already a sheet. |
...
| Who owns this transaction (few members) | Radio rows |
| Ledger vs recurring; portfolio vs sandbox | Tabs (same object, alternate views) |
| Report time window (few presets) | Chip / segmented |
| Custom date range (reports or ledger filters) | Two DateFields; calendar overlays the already-open sheet |
| Ledger period (few presets) | Exclusive chips (radiogroup) |
| Clear advanced ledger filters | Tertiary text on the toolbar |
| Transaction type (in / out / transfer) | Radio group under the amount |
| Account type (4) | Radio cards with helper text |
| Liquidity (spendable / reserve) | Radio group |
| Budget cadence (monthly / annually) | Radio group |
| Holding currency (EUR / USD) | Radio group |
| Recurrence frequency | Radio group |
| Category (long) | Combobox (`CategorySelect`) |
| Share ticker (unknown set) | Combobox (`ShareSelector`) |
| Theme light/dark | Switch (immediate) |
| Import bank CSV | Settings row → labelled native file input |
| Currency, language, timezone | Searchable select in a sheet |
| Sandbox forecast numbers | Label-above text + `inputMode` (not `type="number"`) |
